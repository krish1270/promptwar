...
"""
# Inject the HTML silently (height=0)
...
Keep responses under 3 sentences. You MUST end your response with exactly one of: [COUNTER], [ACCEPTED], or [REJECTED]."""
        ...
        buyer_model = genai.GenerativeModel('gemini-1.5-flash', system_instruction=buyer_system)
        seller_model = genai.GenerativeModel('gemini-1.5-flash', system_instruction=seller_system)
        ...
        for round_num in range(1, 5): 
            ...
            seller_response = seller_model.generate_content(f"Buyer just said: {current_offer}\nRespond:").text
            ...
            buyer_response = buyer_model.generate_content(f"Seller just said: {seller_response}\nRespond:").text
        ...
        if status == "Accepted":
            ...
            contract_model = genai.GenerativeModel('gemini-1.5-flash')
            agreement_prompt = f"Based on this B2B negotiation transcript, generate a formal, final contract.\n\nTranscript:\n{transcript}"
            agreement = contract_model.generate_content(agreement_prompt).text
